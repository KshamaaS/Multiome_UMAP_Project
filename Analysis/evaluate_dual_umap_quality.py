...
Usage:
    python evaluate_dual_umap_quality.py
"""
import os
import sys
import numpy as np
...
# Setup paths
RESULTS_DIR = '/content/drive/MyDrive/Multiome_UMAP_Project/results/gse140203_results'
...
# Load results
print("\nLoading processed data with UMAPs...")
data_file = os.path.join(RESULTS_DIR, 'processed_mdata.h5mu')

if not os.path.exists(data_file):
    print(f"\n❌ ERROR: Data file not found at: {data_file}")
    print("\nPlease ensure:")
    print("  1. You've run the preprocessing pipeline")
    print("  2. The path above points to your results directory")
    sys.exit(1)

mdata = md.read_h5mu(data_file)
...
# Save summary
summary_df.to_csv(os.path.join(EVAL_DIR, 'evaluation_metrics.csv'), index=False)
...
plt.tight_layout()
plt.savefig(os.path.join(EVAL_DIR, 'silhouette_by_celltype.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(EVAL_DIR, 'cluster_comparison.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(EVAL_DIR, 'metrics_comparison.png'), dpi=300, bbox_inches='tight')
plt.close()
